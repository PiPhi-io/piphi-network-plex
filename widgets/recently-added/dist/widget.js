...
async function mountLiveMediaWidget(host, root, timer = globalThis) {
  ...
  function render(items) {
    ...
    for (const item of items) {
      ...
      meta.className = "meta";
      meta.textContent = [item.kind, item.year, item.artist].filter(Boolean).join(" \xB7 ");
  ...
  async function refresh() {
    ...
    if (failures) {
      ...
      status.textContent = "Reconnecting to Plex\u2026";
    }
    try {
      ...
      status.textContent = `Live \xB7 updated ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}`;
    } catch (error) {
      if (disposed) return;
      failures++;
      ...
      status.textContent = hasData ? "Connection lost \xB7 showing previous results. Retrying\u2026" : "Unable to load Plex. Check access, server, and widget settings. Retrying\u2026";
    } finally {
      busy = false;
